...
import { technologies, getAvailableTechs } from '../../game/gameData';
import { canBuyTech } from '../../game/gameState';
import { Check, Lock, List, GitBranch } from 'lucide-react';
...
export default function TechnologyPanel({ player, playsRemaining, onBuyTech }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-1 bg-secondary/40 rounded-lg p-0.5">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1 py-1.5 rounded-md text-xs font-bold transition-colors
            ${!showTree ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
        >
      ...
      {/* Tree view */}
      {showTree && (
        <TechTreeView player={player} playsRemaining={playsRemaining} onBuyTech={onBuyTech} />
      )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {tech.deadEnd && (
                            <span className="text-[9px] bg-amber-500/30 text-amber-300 border border-amber-500/40 rounded-full px-1.5 py-0.5 font-bold">
                              ⭐ {lang === 'ko' ? '보너스' : 'Bonus'}
                            </span>
                          )}
                        </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {ownedTechs.map(tech => (
                  ...
                  >
                    ...
                    {tech.deadEnd && <span className="text-[9px] text-amber-400">⭐</span>}
